...
class GetNearbyPlacesTest : BaseUseCaseTest() {
    ...
    @Test
    fun `Given GetNearbyPlaces execution, When latitude, longitude and placeType are passed, Then getNearbyDetail from repository is invoked with passed latitude, longitude and placeType`() {
    ...
    fun `Given GetNearbyPlaces execution, When list of places is returned by the repository, Then same list is returned by the useCase`() {
        val placeDomainList: MutableList<PlaceDomain> = mutableListOf<PlaceDomain>()
        placeDomainList.add(
            PlaceDomain(
                "id", "",
                "", 0, "",
